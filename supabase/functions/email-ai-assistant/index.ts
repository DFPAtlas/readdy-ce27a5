...
function validateAIResponse(response: unknown, capability: string): { valid: boolean; result: unknown; warnings: string[] } {
  ...
  if (capability === "draft_from_brief") {
    if (resp.sections && Array.isArray(resp.sections)) {
      resp.sections = (resp.sections as Array<Record<string, unknown>>).map(s => ({
        ...s,
        paragraphs: Array.isArray(s.paragraphs) ? s.paragraphs.map((p: unknown) => typeof p === 'string' ? sanitizeContent(p) : p) : s.paragraphs,
      }));
    }
...
function buildContentForModel(capability: string, payload: Record<string, unknown>, systemPrompt: string): { messages: Array<{role: string; content: string}> } {
  ...
  switch (capability) {
    case "subject_lines":
      messages.push({
        role: "user",
        content: `Generate ${payload.count || 5} subject line options for an email with the following context:\n\nPurpose: ${payload.purpose || "Not specified"}\nAudience: ${payload.audience || "Not specified"}\nCategory: ${payload.category || "marketing"}\nCurrent subject (for reference): ${payload.current_subject || "None"}\nBrand: ${payload.brand_name || "Not specified"}\n\nFor each option, provide: the subject line text, character count, tone, merge tags used (if any), and any potential issues. Format as JSON with subject_options array.`
      });
      break;
    case "preview_text":
      messages.push({
        role: "user",
        content: `Generate ${payload.count || 3} preview text options for this email. The preview text should complement, not repeat, the subject line.\n\nSubject: ${payload.subject || ""}\nCurrent preview: ${payload.current_preview || "None"}\nCategory: ${payload.category || "marketing"}\n\nFor each option, provide: the preview text, character count, and how it pairs with the subject. Format as JSON with preview_options array.`
      });
      break;
    case "rewrite_text":
      messages.push({
        role: "user",
        content: `Rewrite the following text: "${payload.text}"\n\nInstruction: ${payload.instruction || "improve clarity"}\nContext: ${payload.context || ""}\n\nProvide: the suggested version, a brief summary of key changes made. Format as JSON with suggested_text and changes_summary fields.`
      });
      break;
    case "draft_from_brief":
      messages.push({
        role: "user",
        content: `Create an email draft from this brief:\n\nPurpose: ${payload.purpose || ""}\nAudience: ${payload.audience || ""}\nBrand: ${payload.brand_name || ""}\nCategory: ${payload.category || "marketing"}\nDesired Action: ${payload.desired_action || ""}\nTone: ${payload.tone || "professional"}\nKey Information: ${payload.key_info || ""}\nRequired Links: ${payload.required_links || ""}\nMerge Tags: ${payload.merge_tags || ""}\nLength: ${payload.length || "medium"}\nRestrictions: ${payload.restrictions || ""}\n\nGenerate: subject_options (3-5), preview_text_options (2-3), outline (array of section headings), draft body with sections, primary CTA, and optional alternative version. Format as JSON.`
      });
      break;
    case "cta_suggestions":
      messages.push({
        role: "user",
        content: `Generate call-to-action button label and supporting copy options.\n\nContext: ${payload.context || ""}\nCurrent CTA: ${payload.current_cta || "None"}\nDesired action: ${payload.desired_action || ""}\n\nProvide 3-5 options. For each: button label, supporting line, and a note about effectiveness. Format as JSON with cta_options array.`
      });
      break;
    case "content_review":
      messages.push({
        role: "user",
        content: `Review the following email content and return improvement suggestions:\n\nSubject: ${payload.subject || ""}\nPreview: ${payload.preview_text || ""}\nContent: ${payload.content || ""}\n\nCheck for: clarity, tone consistency, repetition, long paragraphs, weak CTAs, missing context, reading difficulty, accessibility issues, image alt text gaps, link wording, subject/body consistency, unsupported claims, mobile readability, missing plain-text content.\n\nFormat as JSON with suggestions array, each having type, severity, title, description, and block_reference (if applicable).`
      });
      break;
    case "accessibility_suggestions":
      messages.push({
        role: "user",
        content: `Review this email content for accessibility improvements:\n\n${payload.content || ""}\n\nSuggest: clearer headings, shorter paragraphs, descriptive links, better alt-text, reduced visual instructions, simpler language, meaningful button text. Format as JSON with suggestions array.`
      });
      break;
    case "tone_consistency":
      messages.push({
        role: "user",
        content: `Check this email content for tone consistency with brand guidelines.\n\nContent: ${payload.content || ""}\nExpected tone: ${payload.expected_tone || "professional"}\n\nFlag inconsistencies and suggest alternatives. Format as JSON with issues array.`
      });
      break;
    case "content_variation":
      messages.push({
        role: "user",
        content: `Create a controlled A/B variation:\n\nElement to vary: ${payload.element || ""}\nOriginal: ${payload.original || ""}\nContext: ${payload.context || ""}\n\nPreserve: links, merge tags, brand and legal content. Label what changed. Format as JSON.`
      });
      break;
    case "content_adaptation":
      messages.push({
        role: "user",
        content: `Adapt this content for a different brand/product:\n\nContent: ${payload.content || ""}\nSource brand: ${payload.source_brand || ""}\nTarget brand: ${payload.target_brand || ""}\nTarget product: ${payload.target_product || ""}\nTarget tone: ${payload.target_tone || ""}\n\nPreserve factual meaning. Only change brand-specific elements. Format as JSON.`
      });
      break;
    case "plain_text":
      messages.push({
        role: "user",
        content: `Convert this HTML email content to a clean plain-text version:\n\n${payload.content || ""}\n\nPreserve links as URLs in brackets. Maintain logical structure with clear section breaks. Format as JSON with plain_text field.`
      });
      break;
    ...
    case "expand":
      messages.push({
        role: "user",
        content: `${capability.replace(/_/g, " ")} this text:\n\n"${payload.text || ""}"\n\nContext: ${payload.context || ""}\n\nProvide the improved version. Format as JSON with suggested_text field.`
      });
      break;
    default:
      messages.push({
        role: "user",
        content: `${payload.prompt || "Generate email content based on the provided context."}`
      });
  }
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { capability, payload, organisation_id, template_id, campaign_id, brand_id } = body;
    ...
    if (!aiApiKey) {
      return new Response(JSON.stringify({ error: "AI_PROVIDER_API_KEY not configured. Add it in Supabase Dashboard > Edge Function Secrets." }), {
    ...
    if (orgOverride || organisation_id) {
      const orgId = orgOverride || organisation_id;
      const { data: settings } = await supabase.from("email_studio_settings")
        .select("ai_config").maybeSingle();
      
      if (settings?.ai_config && typeof settings.ai_config === "object") {
        const aiConfig = settings.ai_config as Record<string, unknown>;
        if (aiConfig.provider && typeof aiConfig.provider === "string") modelConfig.provider = aiConfig.provider;
        if (aiConfig.model && typeof aiConfig.model === "string") modelConfig.model = aiConfig.model;
        if (aiConfig.base_url && typeof aiConfig.base_url === "string") modelConfig.base_url = aiConfig.base_url;
        if (typeof aiConfig.max_input_tokens === "number") modelConfig.max_input_tokens = aiConfig.max_input_tokens;
        if (typeof aiConfig.max_output_tokens === "number") modelConfig.max_output_tokens = aiConfig.max_output_tokens;
        if (typeof aiConfig.temperature === "number") modelConfig.temperature = aiConfig.temperature;
        if (typeof aiConfig.request_timeout_ms === "number") modelConfig.request_timeout_ms = aiConfig.request_timeout_ms;
        
        if (aiConfig.ai_enabled === false) {
          return new Response(JSON.stringify({ error: "AI assistant is disabled for this organisation" }), {
    ...
    if (brandId) {
      const { data: brand } = await supabase.from("email_brand_kits")
        .select("name, product_name, description, ai_voice_settings")
    ...
    const timeoutId = setTimeout(() => controller.abort(), modelConfig.request_timeout_ms);
    ...
    try {
      ...
      return new Response(JSON.stringify({ error: isTimeout ? "AI request timed out" : `AI provider unavailable: ${(fetchErr as Error).message}` }), {
    ...
    if (!aiResponse.ok) {
      ...
      let errMsg = `AI provider error (${aiResponse.status})`;
      if (aiResponse.status === 429) errMsg = "Rate limit exceeded. Try again shortly.";
      else if (aiResponse.status === 401) errMsg = "AI provider authentication failed. Check API key.";
      try {
    ...
    try {
      ...
      parsed = { raw_response: aiContent };
    }

    const validation = validateAIResponse(parsed, capability);
    if (!validation.valid) {
      return new Response(JSON.stringify({ error: validation.warnings.join(". ") }), {
    ...
    try {
      await supabase.from("email_ai_usage_logs").insert({
        ...
        template_id: template_id || payload.template_id || null,
    ...
    return new Response(JSON.stringify({
      ...
      usage: { input_units: inputUnits, output_units: outputUnits, estimated_cost: estimatedCost },
